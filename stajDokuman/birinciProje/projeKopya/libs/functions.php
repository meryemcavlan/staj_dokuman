...
function isAdmin() {
    return (isLoggedIn() && isset($_SESSION["user_type"]) && $_SESSION["user_type"] == 1);    
}
...
function getCoursesByFilters($categoryId, $keyword, $page) {
    ...
    if(!empty($keyword)) {
        $query = " from kurslar WHERE baslik LIKE '%$keyword%' or altBaslik LIKE '%$keyword%'";
    }

    $total_sql = "SELECT COUNT(*) ".$query;
    $count_data = mysqli_query($baglanti,$total_sql);
    $count = $count_data->fetch_array()[0];
